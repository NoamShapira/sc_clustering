...
def pre_procces_adata(adata: ad.AnnData):
    ...
    sc.pp.calculate_qc_metrics(adata, config.PP_QUALITY_CONTROL_PARAMS)
    ...
    # compute genes variation
    sc.pp.highly_variable_genes(adata, **config.PP_HIGHLY_VARIATION_PARAMS)

    # save procced data in adata.raw
    adata.raw = adata
