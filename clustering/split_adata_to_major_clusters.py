...
def split_to_major_cluster(adata_with_scvi_emb: ad.AnnData, emb_col_name: str,
                           clustering_model_name: str, num_clusters: int,
                           col_name_to_split_by:str="major_clusters") -> List[ad.AnnData]:
    ...
    if clustering_model_name == "kmeans":
        ...
        adata_with_scvi_emb.obs[col_name_to_split_by] = clustering_model.fit(
            adata_with_scvi_emb.obsm[emb_col_name]).labels_
        ...
        sc.pp.neighbors(adata_with_scvi_emb, use_rep=emb_col_name)
        leiden_col_name = "leiden_for_major_cluster"
        sc.tl.leiden(adata_with_scvi_emb, resolution=config.TL_LEIDEN_RESOLUTION * 1.5, key_added=leiden_col_name)

        all_cells_df = pd.DataFrame(index=adata_with_scvi_emb.obs_names,
                                    data=adata_with_scvi_emb.obsm[emb_col_name])
        all_cells_df[leiden_col_name] = adata_with_scvi_emb.obs[leiden_col_name]
        ...
        adata_with_scvi_emb.obs[col_name_to_split_by] = cells_cluster
    else:
