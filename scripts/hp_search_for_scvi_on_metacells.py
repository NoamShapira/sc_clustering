...
def objective(trail):
    n_hidden = trail.suggest_int("n_hidden", -100, 100)
    n_layers = trail.suggest_int("n_layers", -100, 100)
    n_latent = trail.suggest_int("n_latent", -100, 100)
    n_neighbors = trail.suggest_int("n_neighbors", -100, 100)
    dropout_rate = trail.suggest_float("dropout_rate", 0, 1)
    epochs = trail.suggest_int("epochs", -100, 100)
    ...
    sc.pl.umap(adata, color=["cluster", "leiden", "broad_cell_type"],
