...
def full_pipeline_and_load_meta_cell_cached(experiments_data_dir=config.UMI_DIR_PATH,
                                            meta_data_path=config.META_DATA_PATH,
                                            path_to_meta_cell_results=config.META_CELL_PATH):
    return run_full_pipeline_and_load_meta_cell(experiments_data_dir, meta_data_path, path_to_meta_cell_results)


def compute_and_plot_rank_genes_groups_to_local_path(adata, groupby, groups, method, n_genes, prefix, *kwargs) -> str:
...
def visualize_meta_cell(adata, meta_cell):
    ...
    adata.obs[MetaCellResultsColumnsNames().meta_cell] = adata.obs[MetaCellResultsColumnsNames().meta_cell].astype(
        'str')
    image_path = compute_and_plot_rank_genes_groups_to_local_path(adata, groupby=MetaCellResultsColumnsNames().meta_cell
...
def visualize_clusters_assosiated_with_meta_cell(adata, meta_cell):
    ...
    image_path_2 = compute_and_plot_rank_genes_groups_to_local_path(adata_only_cluster, groupby=MetaCellResultsColumnsNames().meta_cell,
                                                                    groups=[meta_cell], method='wilcoxon',
